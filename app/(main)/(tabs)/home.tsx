...
import { fetchUserStatus } from "@/lib/actions/points"
import type { Game, GameFilterOptions, Team, UserStatusWithLevel } from "@/types/updated_types"
import { sortGamesByPriority } from "@/utils/sortGame"
import { getLiveGames, getPastGames, getTeams, getUpcomingGames } from "@/version_control/actions/index"; // Declare the variables here
import { useSync, useSyncStatus } from "@/version_control/index"
...
export default function HomeScreen() {
  ...
  // Consolidated data fetching function using current-sync
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      console.log("📱 Loading data from current-sync...")

      // Fetch data from current-sync (cached data)
      const [upcoming, past, live, teams] = await Promise.all([
        getUpcomingGames(20),
        getPastGames(10),
        getLiveGames(5),
        getTeams(),
      ...
      // Still fetch user status from server (user-specific data)
      if (userId) {
  ...
  const getSyncStatusMessage = () => {
    ...
    if (syncStatus.pendingChanges > 0) {
      return `${syncStatus.pendingChanges} updates pending`
    }
              ...
              <View style={styles.connectionStatus}>
                <View
                  style={[
                    ...
                    {
                      backgroundColor: syncStatus.isInProgress
                        ? "#FF9500"
                ...
                />
                <Text style={styles.connectionText}>{getSyncStatusMessage()}</Text>
              </View>
            ...
            <View style={styles.sectionTitleContainer}>
              ...
              <View>
                ...
                <Text style={styles.sectionSubtitle}>
                  {loading ? "Loading..." : `${allTeams.length} teams available`}
                </Text>
              </View>
            </View>
        ...
        <View style={styles.section}>
          ...
          {loading ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Loading games from cache...</Text>
            </View>
